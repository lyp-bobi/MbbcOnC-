...
int main() {
    ...
    //    double seglenParas[]={100,150,200,250,300,400,500,600,700,800,900,1000};//for td
        ...
        for(double seglen:seglenParas){
            maxseg=std::max(maxseg,Trajectory::cutTrajsIntoFile(trajs,seglen,0,"glopts"+to_string(seglen)+".stj"));
        }
...
//        double seglenParas[]={20,50,80,120,160,200,300,500};// for GeoLife
        ...
        for(double seglen:seglenParas){
            maxseg=std::max(maxseg,Trajectory::cutTrajsIntoFile(trajs,seglen,0,"tdopts"+to_string(seglen)+".stj"));
        }
