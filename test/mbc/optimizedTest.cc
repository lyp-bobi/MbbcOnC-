    ...
    for(auto id:ids){
        ...
        if(traj.size()>=10){
            ...
            for(int l=0;l<traj.size();l++){
                ...
                if(l!=0){
                    dist+=std::sqrt(sq(traj[l].x-lastpoint.x)+sq(traj[l].y-lastpoint.y));
                }
                lastpoint=traj[l];
            }
            if(!tps.empty()){
...
double knncost(double bt,int k,double qt,int f,bool useMBR){
    auto stat=trajStat::instance();
...
double biSearchMax(int k,double qt,int f,bool useMBR){
    double low=1,high=500;
    std::stack<d4> st;
    double vlow,vhigh;
    vlow=knncost(low,k,qt,f,useMBR);
    vhigh=knncost(high,k,qt,f,useMBR);
    ...
    while(!st.empty()){
        ...
        double vmid=knncost(mid,k,qt,f,useMBR);
        bool rising=knncost(mid+0.1,k,qt,f,useMBR)>vmid;
//        std::cerr<<mid<<"\t"<<vmid<<"\n";
        if(rising){
