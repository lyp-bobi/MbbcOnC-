...
uint32_t Node::getByteArraySize(){
    return
            (sizeof(uint32_t) +
             sizeof(uint32_t) +
             sizeof(uint32_t) +
             (m_children * (m_nodeMBBCk.getByteArraySize() + sizeof(id_type) + sizeof(uint32_t))) +
             m_totalDataLength +
                    m_nodeMBBCk.getByteArraySize());
}

void Node::loadFromByteArray(const uint8_t* ptr){

    m_nodeMBBCk=m_pTree->m_infiniteMBBCk;
    ...
    {
        m_ptrMBBCk[u32Child] = m_pTree->m_MBBCkPool.acquire();
        *(m_ptrMBBCk[u32Child]) = m_pTree->m_infiniteMBBCk;

        m_ptrMBBCk[u32Child]->loadFromByteArray(ptr);
        ptr += m_ptrMBBCk[u32Child]->getByteArraySize();
    ...
    m_nodeMBBCk.loadFromByteArray(ptr);
...
void Node::storeToByteArray(uint8_t** data, uint32_t& len){
    ...
    *data = new uint8_t[len];
    ...
    for (uint32_t u32Child = 0; u32Child < m_children; ++u32Child)
    {
        ...
        m_ptrMBBCk[u32Child]->storeToByteArray(&tmpb,tmplen);
    ...
    m_nodeMBBCk.storeToByteArray(&tmpb,tmplen);
    memcpy(ptr, tmpb, tmplen);
    //ptr += tmplen;
...
void Node::getChildShape(uint32_t index, IShape** out) const
{
    ...
    *out = new MBBCk(*(m_ptrMBBCk[index]));
}
...
Node::Node(SpatialIndex::PAAR2Tree::PAAR2Tree* pTree, id_type id, uint32_t level, uint32_t capacity) :
        m_pTree(pTree),
        m_level(level),
        m_identifier(id),
        m_children(0),
        ...
        m_totalDataLength(0)
{
    m_nodeMBBCk.makeInfinite(m_pTree->m_dimension,m_pTree->m_k);

    try
    {
        m_pDataLength = new uint32_t[m_capacity + 1];
        m_pData = new uint8_t*[m_capacity + 1];
        m_ptrMBBCk = new MBBCkPtr[m_capacity + 1];
...
void Node::insertEntry(uint32_t dataLength, uint8_t* pData, MBBCk& MBBCk, id_type id)
{
    assert(m_children < m_capacity);

    m_pDataLength[m_children] = dataLength;
    m_pData[m_children] = pData;
    m_ptrMBBCk[m_children] = m_pTree->m_MBBCkPool.acquire();
    *(m_ptrMBBCk[m_children]) = MBBCk;
    m_pIdentifier[m_children] = id;

    m_totalDataLength += dataLength;
    ++m_children;

    m_nodeMBBCk.combineMBBCk(MBBCk);
}
...
bool Node::insertData(uint32_t dataLength, uint8_t* pData, MBBCk& mbbc, id_type id, std::stack<id_type>& pathBuffer, uint8_t* overflowTable)
{
    if (m_children < m_capacity)
    {
        ...
        bool b = m_nodeMBBCk.containsMBBCk(mbbc);
