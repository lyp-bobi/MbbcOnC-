...
int TIME_SLAB = 60;
supported_distance current_distance = IED;
...
xTrajectory::xTrajectory(bool fakehead, bool fakeback,
                         std::vector<SpatialIndex::xPoint> &in) {
#ifndef NDEBUG
...
uint32_t xTrajectory::getByteArraySize() const {
    return 2 * sizeof(bool) + sizeof(unsigned long) +
           (m_dimension + 1) * sizeof(double) * m_points.size();
}
...
xPoint xTrajectory::getPointAtTime(const double time) const {
    if (time < m_points.front().m_t || time > m_points.back().m_t) {
        throw Tools::IllegalArgumentException(
                "xTrajectory::getPointAtTime: time" + std::to_string(time) +
                "is illegal."
        );
    }
    if (m_points.size() == 1) {
...
xTrajectory xTrajectory::resampleQuery(double ts, double te, int nsegment) const {
    ...
    for (int i = 0; i <= nsegment; i++) {
        ...
        if (t < m_startTime()) continue;
        if (t > m_endTime()) break;
        pnts.emplace_back(getPointAtTime(t));
    }
    return xTrajectory(pnts);
}
...
bool xTrajectory::intersectsxCylinder(const SpatialIndex::xCylinder &in) const {
    ...
    for (int i = 0; i < timedTraj.m_size - 1; i++) {
        if (line2lineMinSED(timedTraj[i], timedTraj[i + 1],
                            ...
                            xPoint(in.m_p.m_x, in.m_p.m_y,
                                   timedTraj[i + 1].m_t)) < in.m_r) {
            return true;
        }
    }
    return false;
...
void xTrajectory::getxMBC(SpatialIndex::xMBC &out) const {
    if (m_points.size() <= 1) {
        std::cerr
                << "WARNING: getting MBC at a xTrajectory with 0 or 1 points\n";
    ...
    double avx = (endx - startx) / (endt - startt), avy =
            (endy - starty) / (endt - startt);
...
findMaximumDistance(const vector<SpatialIndex::xPoint> &points) {
    ...
    for (int i = 1; i < points.size() -
                        1; i++) { //traverse through second point to second last point
        double Dist = SpatialIndex::xPoint::makemid(firxPoint, laxPoint,
                                                    points[i].m_t).getMinimumDistance(
                points[i]);
        if (Dist > Mdist) {
            Mdist = Dist;
            index = i;
...
std::vector<SpatialIndex::xPoint>
xTrajectory::simplifyWithRDP(const std::vector<SpatialIndex::xPoint> &Points,
                             double epsilon) {
    ...
    if (maxDistance.second >= epsilon) {
        ...
        vector<SpatialIndex::xPoint> path1(Points.begin(), it + index +
                                                           1); //new path l1 from 0 to index
        vector<SpatialIndex::xPoint> path2(it + index,
...
std::vector<std::vector<SpatialIndex::xPoint>>
xTrajectory::simplifyWithRDPN(const std::vector<SpatialIndex::xPoint> &Points,
                              int numPart) {
    ...
    while (paths.size() < numPart) {
        ...
        vector<SpatialIndex::xPoint> path1(it1, it1 + placeIndex +
                                                1); //new path l1 from 0 to index
        vector<SpatialIndex::xPoint> path2(it1 + placeIndex,
                                           it2); // new path l2 from index to last
...
std::vector<xTrajectory>
xTrajectory::cuttraj(std::vector<SpatialIndex::xPoint> mask) {
    vector<xPoint> seg;
...
std::vector<xTrajectory> xTrajectory::getRDPSegments(double len) const {
    ...
    auto m = simplifyWithRDPN(m_points,
                              std::min(int(m_points.size() - 1), segNum));
    for (auto &pts:m) {
        if (pts.size() < 2) {
...
//    std::cerr<<int(std::floor(std::pow(segNum,1.0/4)))<<"\t";
    auto m = simplifyWithRDPN(m_points,
                              std::min(int(std::sqrt(m_points.size() - 1)),
                                       int(segNum / 5)));
    for (auto &pts:m) {
        if (pts.size() < 2) {
            std::cerr << "error on getting segments with " << len << "and \n"
...
std::vector<xTrajectory> xTrajectory::getStaticSegmentsCut(double len) const {
    ...
    for (int i = 1; i < m_points.size(); i++) {
        if (m_points[i].m_t < segStart + len) {
            ...
            xPoint mid = xPoint::makemid(m_points[i - 1], m_points[i],
                                         segStart + len);
            fakeback = true;
...
inline prec theF(prec c1, prec c2, prec c3, prec c4, prec t) {
    ...
    if (delta <= 1e-10) {
        return (2 * c1 * t + c2) *
               sqrtp(std::max((prec) 0.0, c1 * t * t + c2 * t + c3)) / 4 / c1 /
               c4;
    } else {
        return asinhp((2 * c1 * t + c2) / sqrtp(delta)) * delta / 8 / c1 /
               sqrtp(c1) / c4
               + (2 * c1 * t + c2) *
                 sqrtp(std::max((prec) 0.0, c1 * t * t + c2 * t + c3)) / 4 /
                 c1 / c4;
    }
}
...
double xTrajectory::line2lineIED(const SpatialIndex::xPoint &p1s,
                                 const SpatialIndex::xPoint &p1e,
                                 const SpatialIndex::xPoint &p2s,
                                 const SpatialIndex::xPoint &p2e) {
    if (p1s.m_t != p2s.m_t | p1e.m_t != p2e.m_t)
        throw Tools::IllegalStateException(
                "line2lineIED: time period not the same");
    ...
    prec c1 = sq(dxs - dxe) + sq(dys - dye),
            c2 = 2 * ((dxe * ts - dxs * te) * (dxs - dxe) +
                      (dye * ts - dys * te) * (dys - dye)),
    ...
    if (c1 < 1e-9) {
        ...
        return min(ub, (double) (theF(c1, c2, c3, c4, te) -
                                 theF(c1, c2, c3, c4, ts)));
    }
}


double xTrajectory::line2lineMinSED(const SpatialIndex::xPoint &p1s,
                                    const SpatialIndex::xPoint &p1e,
                                    const SpatialIndex::xPoint &p2s,
                                    const SpatialIndex::xPoint &p2e) {
    if (p1s.m_t != p2s.m_t | p1e.m_t != p2e.m_t)
        throw Tools::IllegalStateException(
                "line2lineMinSED: time period not the same");
    ...
    if (c1 < 1e-7) {
        ...
        if (middle > ts && middle < te) {
            ...
            return std::min(std::sqrt(sq(dxs) + sq(dys)),
                            std::sqrt(sq(dxe) + sq(dye)));
        }
    }
}
...
xPoint *
cutByLine(const SpatialIndex::xPoint &ps, const SpatialIndex::xPoint &pe,
          double value, int axis) {
    int otheraxis = 1 - axis;
    double axisv1 = ps.m_pCoords(axis), axisv2 = pe.m_pCoords(axis);
    if ((axisv1 < value) == (axisv2 < value) ||
        std::fabs(axisv1 - value) < 1e-7 ||
        std::fabs(axisv2 - value) < 1e-7) //need no cut
        return nullptr;
    else {
        ...
        if (axis == 0) {
            xyt[0] = value;
            xyt[1] =
                    d2 * ps.m_pCoords(otheraxis) + d1 * pe.m_pCoords(otheraxis);
...
std::vector<std::pair<xPoint, xPoint>>
xTrajectory::cutByPhase(const SpatialIndex::xPoint &ps,
                        const SpatialIndex::xPoint &pe,
...
inline double xTrajectory::line2MBRMinSED_impl(const SpatialIndex::xPoint &ps,
                                               const SpatialIndex::xPoint &pe,
                                               const SpatialIndex::xMBR &r,
                                               int sr) {
...
double xTrajectory::line2MBRMinSED(const SpatialIndex::xPoint &ps,
                                   const SpatialIndex::xPoint &pe,
                                   const SpatialIndex::xMBR &r) {
...
inline double xTrajectory::line2MBRIED_impl(const SpatialIndex::xPoint &ps,
                                            const SpatialIndex::xPoint &pe,
                                            const SpatialIndex::xMBR &r,
                                            int sr) {
    double ts = ps.m_t, te = pe.m_t;
    ...
    if (sr == 5) return 0;
        ...
        return 0.5 * (ps.getMinimumDistance(r) + pe.getMinimumDistance(r)) *
               (te - ts);
    } else {
        double px, py;
        if (sr == 1 || sr == 7) px = r.m_xmin;
...
inline DISTE xTrajectory::line2MBRIED(const SpatialIndex::xPoint &ps,
                                      const SpatialIndex::xPoint &pe,
                                      const SpatialIndex::xMBR &r) {
...
//            cout<<tmpsr<<" "<<tmpres<<"\n";
    ...
    double pessi = opti + (pe.m_t - ps.m_t) * sqrt(sq(r.m_xmax - r.m_xmin) +
                                                   sq(r.m_ymax - r.m_ymin));
    return DISTE(opti, pessi, false);
}

inline double
mbcArea(double t0, double t3, double rd, double rv, double ts, double te) {
...
inline DISTE xTrajectory::line2MBCIED(const SpatialIndex::xPoint &ps,
                                      const SpatialIndex::xPoint &pe,
    ...
    double ratio1 = (ps.m_t - t1) / (t2 - t1), ratio2 =
            (pe.m_t - t1) / (t2 - t1);
...
inline DISTE xTrajectory::line2MBLIED(const SpatialIndex::xPoint &ps,
                                      const SpatialIndex::xPoint &pe,
...
#define half_adjust(f) ((int)((f) + 0.5))
...
xTrajectory::getMinimumDistance(const SpatialIndex::xTrajectory &in) const {
    if (current_distance == RMDTW) {
        ...
        subin = subin.resampleQuery(m_startTime(), m_endTime(),
                                    m_points.size() - 1);
        double interval = (m_endTime() - m_startTime()) / (m_points.size() - 1);
        int padprev = 0, padnext = 0;
        if (subin.m_points.size() != m_points.size()) {
            if (in.m_startTime() > m_startTime())
                padprev = ceil((in.m_startTime() - m_startTime()) / interval);
            if (in.m_endTime() < m_endTime())
                padnext = ceil((m_endTime() - in.m_endTime()) / interval);
            assert(m_points.size() ==
                   subin.m_points.size() + padprev + padnext);
        }
        vector<vector<double>> ps1, ps2;
        ps1.resize(m_points.size());
        ps2.resize(subin.m_points.size() + 2 * padnext + 2 * padprev);
        for (int i = 0; i < m_points.size(); i++) {
        ...
        for (int j = 0; j < padprev * 2; j++) {
            ps2[pn].emplace_back(in.m_points.front().m_x);
            ps2[pn++].emplace_back(in.m_points.front().m_y);
        }
        for (int i = 0; i < subin.m_points.size(); i++) {
            ps2[pn].emplace_back(subin.m_points[i].m_x);
            ps2[pn++].emplace_back(subin.m_points[i].m_y);
        }
...
xTrajectory::getPartialRMDTW(const SpatialIndex::xTrajectory &parttraj) const
{
    ...
    for (auto &p: m_points) {
        ...
        double maxdist = 0;
        xPoint tp2 = parttraj.getPointAtTime(p.m_t);
        for (auto &q:m_points) {
            xPoint tp = q;
            tp.m_t = p.m_t;
            double d = tp.getMinimumDistance(tp2);
            if (d < mindist)
                mindist = d;
            if (d > maxdist)
                maxdist = d;
        }
        DISTE add = DISTE(mindist, maxdist, false);
        res += add;
        npoint++;
    }
...
double
xTrajectory::getStaticDistance(double x, double y, double t1, double t2) const {
    double tstart, tend;
    ...
    if(current_distance != IED) {
        ...
        double mindist = 1e300;
        for (auto &p:m_points) {
            double d = p.getMinimumDistance(pstatic);
...
double xTrajectory::getStaticDistance(SpatialIndex::xMBR in, double ints,
                                      double inte) const {
#ifndef NDEBUG
    if (ints > m_startTime() && inte < m_endTime()) {
        throw Tools::IllegalStateException(
                "ints>=m_startTime()&&inte<=m_endTime()");
    }
#endif
    in.m_tmin = ints;
    in.m_tmax = inte;
    ...
    if (current_distance == RMDTW)
    {
        ...
        double mindist = 1e300;
        for (auto &p:m_points) {
            double d = p.getMinimumDistance(in);
            if (d < mindist) mindist = d;
        }
        return mindist * nslab * 2;
    } else {
        ...
        for (int i = 0; i < timedTraj.m_size - 1; i++) {
            ...
            double pd = line2MBRIED(timedTraj[i], timedTraj[i + 1],
                                    in).opt;
            sum += pd;
        }
        return sum;
...
int xTrajectory::nPointDuring(double tmin, double tmax) const {
    int res = 0;
    for (auto &p:m_points) {
        if (p.m_t < tmin)
            continue;
...
double xTrajectory::nodeDist(const xSBB &b) const {
    ...
    if (current_distance != IED)
        return m_points.size() * mindist;
    else
        return mindist * (m_endTime() - m_startTime());
}
...
DISTE xTrajectory::sbbDist(const xSBB &b) const {
    ...
    int npoint = 0;
    if (current_distance == RMDTW) {
        if (b.m_startTime >= m_endTime() || b.m_endTime <= m_startTime()) {
            return DISTE(1e300);
        }
        for (auto &p: m_points) {
            if (p.m_t < b.m_startTime) continue;
            if (p.m_t > b.m_endTime) break;
            double mindist = 1e300;
            double maxdist = 0;
            for (auto &q:m_points) {
                xPoint tp = q;
            ...
            DISTE add;
            if (b.hasbl)
                add = DISTE(mindist, maxdist, false);
            else if (b.hasbc)
                add = DISTE(mindist, (maxdist + b.bc.m_rd), false);
            else if (b.hasbr)
                add = DISTE(mindist,
                            (maxdist + b.br.m_xmax - b.br.m_xmin + b.br.m_ymax -
                             b.br.m_ymin), false);
            res += add;
            npoint++;
        }
        ...
        if (b.hasbr) {
            for (int i = 0; i < timedTraj.m_size - 1; i++) {
                res = res + line2MBRIED(timedTraj[i], timedTraj[i + 1], b.br);
            }
        } else if (b.hasbc) {
            for (int i = 0; i < timedTraj.m_size - 1; i++) {
                res = res + line2MBCIED(timedTraj[i], timedTraj[i + 1], b.bc);
            }
        } else if (b.hasbl) {
            for (int i = 0; i < timedTraj.m_size - 1; i++) {
                res = res + line2MBLIED(timedTraj[i], timedTraj[i + 1], b.bl);
            }
        }
    }
...
DISTE xTrajectory::frontDistStatic(const xSBB &b) const {
    if (b.hasbc) {
        return DISTE(
                getStaticDistance(b.bc.m_ps.m_x, b.bc.m_ps.m_y, m_startTime(),
                                  b.bc.m_ps.m_t));
    } else if (b.hasbl) {
        return DISTE(
                getStaticDistance(b.bl.m_ps.m_x, b.bl.m_ps.m_y, m_startTime(),
                                  b.bl.m_ps.m_t));
    } else {
        return DISTE(getStaticDistance(b.br, m_startTime(), b.br.m_tmin));
    }
}
...
DISTE xTrajectory::backDistStatic(const xSBB &b) const {
    if (b.hasbc) {
        return DISTE(
                getStaticDistance(b.bc.m_pe.m_x, b.bc.m_pe.m_y, b.bc.m_pe.m_t,
                                  m_endTime()));
    } else if (b.hasbl) {
        return DISTE(
                getStaticDistance(b.bl.m_pe.m_x, b.bl.m_pe.m_y, b.bl.m_pe.m_t,
                                  m_endTime()));
    } else {
        return DISTE(getStaticDistance(b.br, b.br.m_tmax, m_endTime()));
    }
}
...
DISTE xTrajectory::frontDistStatic(const xPoint &b) const {
    return DISTE(getStaticDistance(b.m_x, b.m_y, m_startTime(), b.m_t), 1e200,
                 false);
}

DISTE xTrajectory::backDistStatic(const xPoint &b) const {
    return DISTE(getStaticDistance(b.m_x, b.m_y, b.m_t, m_endTime()), 1e200, false);
}
...
void xTrajectory::loadFromString(std::string str) {
    ...
    for (const auto &p: points) {
        ...
        m_points.emplace_back(xPoint(std::stod(xyt[0]), std::stod(xyt[1]),
                                     std::stod(xyt[2])));
    }
...
void xTrajectory::linkxTrajectory(SpatialIndex::xTrajectory &other) {
    if (m_points.back().m_t == other.m_points.front().m_t) {
        if (m_fakeback) {
            m_points.pop_back();
            m_points.insert(m_points.end(), ++other.m_points.begin(),
                            other.m_points.end());
        ...
        if (m_fakehead) {
            ...
            m_points.insert(m_points.begin(), other.m_points.begin(),
                            other.m_points.end() - 1);
...
//        std::cerr<<*this<<other<<"\n";
        ...
        throw Tools::IllegalStateException(
                "xTrajectory::linkxTrajectory: the two trajectories to be linked should have a common point.");
    }
}


void xTrajectory::getPartialxTrajectory(double tstart, double tend,
                                        SpatialIndex::xTrajectory &out) const {
    ...
    if (is != 0 && m_points[is].m_t != tstart) {
        double ratio = (tstart - m_points[is - 1].m_t) /
                       (m_points[is].m_t - m_points[is - 1].m_t);
    ...
    if (ie != m_points.size() - 1 && m_points[ie].m_t != tend) {
        double ratio = (tend - m_points[ie].m_t) /
                       (m_points[ie + 1].m_t - m_points[ie].m_t);
...
int xTrajectory::cutTrajsIntoFile(
        std::vector<std::pair<SpatialIndex::id_type, SpatialIndex::xTrajectory>> &trajs,
    ...
    for (int i = 1; i < traj.m_points.size(); i++) {
        if (traj.m_points[i].m_t < segStart + len) {// pass point
            ...
            if (fabs(traj.m_points[i].m_t - segStart - len) < 1e-7) {
                ...
                me = i;
                res.push(make_pair(make_pair(ms, me), subtrajToSBB(subtraj)));
                ms = i;
                fakehead = false;
                seg.clear();
                seg.emplace_back(traj.m_points[i]);
                segStart += len;
            } else if (traj.m_points[i - 1].m_t >
                       segStart + (1 - splitSoftThres) * len) {
                ...
                xPoint mid = xPoint::makemid(traj.m_points[i - 1],
                                             traj.m_points[i], segStart + len);
                fakeback = true;
                seg.emplace_back(mid);
                subtraj = xTrajectory(fakehead, fakeback, seg);
                me = i;
...
static queue<CUTENTRY >
pack_entry(xTrajectory &traj, vector<xTrajectory> &sub) {
    queue<CUTENTRY > res;
    for (auto &s:sub) {
        res.push(make_pair(make_pair(
                traj.locate_time_cut(s.m_startTime(), -1),
                traj.locate_time_cut(s.m_endTime(), 1)), subtrajToSBB(s)));
    }
    return res;
}
...
queue<pair<pair<int, int>, xSBB> >
xTrajectory::OPTS(xTrajectory &traj, double len) {
    int segNum = std::ceil((traj.m_endTime() - traj.m_startTime()) / len);
    queue<CUTENTRY > res;
    if (segNum == 1) {
        res.emplace(make_pair(make_pair(0, int(traj.m_points.size() - 1)),
                              subtrajToSBB(traj)));
        return res;
    }
    int seg1 = std::min(std::ceil(segNum / 5),
                        std::ceil(sqrt(traj.m_points.size() - 1)));
...
queue<CUTENTRY > xTrajectory::FP(xTrajectory &traj, double np) {
    ...
    for (int i = 0; i < traj.m_points.size(); i++) {
        ...
        if (i == traj.m_points.size() - 1) {
            ...
            res.push(make_pair(make_pair(ms, i), subtrajToSBB(subtraj)));
            ms = i;
            seg.clear();
            break;
        }
...
queue<CUTENTRY > xTrajectory::EveryLine(xTrajectory &traj) {
    ...
    for (int i = 0; i < traj.m_points.size() - 1; i++) {
        res.push(make_pair(make_pair(i, i + 1),
                           xSBB(traj.m_points[i], traj.m_points[i + 1])));
    }
    return res;
}
...
queue<pair<pair<int, int>, xSBB> >
xTrajectory::RDP(xTrajectory &traj, double len) {
    ...
    std::vector<std::vector<SpatialIndex::xPoint>> m = simplifyWithRDPN(
            traj.m_points, segNum);
    int pointPrev = 0;
    for (auto &pts:m) {
        xTrajectory subtraj(pts);
        CUTENTRY s;
