...
void PartsStore::Parts::insert(xSBB &r, id_type prev, id_type next,
                               xStoreEntry &entry) {
    is_modified = true;
    ...
    if (prev >= 0 &&
        m_ps->m_loadedLeaf.find(prev) == m_ps->m_loadedLeaf.end())
        m_missingLeaf.insert(prev);
    if (next >= 0 &&
        m_ps->m_loadedLeaf.find(next) == m_ps->m_loadedLeaf.end())
        m_missingLeaf.insert(next);
    m_ses[r.m_startTime] = entry;
}

bool PartsStore::insert(id_type id, xSBB &b, id_type leafid, id_type prev,
                        id_type next, xStoreEntry &entry) {
    bool res = false;
...
//    }
    ...
    if (type == 3 && current_distance == RMDTW) {
        ...
        if (parts->m_completelb.infer) {
            std::sort(parts->m_sbbs.begin(), parts->m_sbbs.end(),
                      [](const xSBB &a, const xSBB &b) {
            ...
            double interval =
                    m_simpquery.m_points[1].m_t - m_simpquery.m_points[0].m_t;
            if (parts->m_sbbs.front().m_startTime > m_simpquery.m_startTime())
                padprev = ceil((parts->m_sbbs.front().m_startTime -
                                m_simpquery.m_startTime()) / interval);
            if (parts->m_sbbs.back().m_endTime < m_simpquery.m_endTime())
                padnext = ceil((m_simpquery.m_endTime() -
                                parts->m_sbbs.back().m_endTime) / interval);
            for (int i = 0; i < padprev; i++) {
                cross.emplace_back(parts->m_sbbs[cur2].crossSec(
                        m_simpquery.m_points.front().m_t));
            }
            for (cur1 = 0; cur1 < m_simpquery.m_points.size(); cur1++) {
                while (parts->m_sbbs[cur2].m_endTime <
                       m_simpquery.m_points[cur1].m_t
                       && cur2 != parts->m_sbbs.size() - 1)
                    cur2++;
                cross.emplace_back(parts->m_sbbs[cur2].crossSec(
                        m_simpquery.m_points[cur1].m_t));
            }
            for (int i = 0; i < padnext; i++) {
                cross.emplace_back(parts->m_sbbs[cur2].crossSec(
                        m_simpquery.m_points.back().m_t));
            }
...
//                    std::cerr<<"load leaf"<<n.m_identifier<<"\n";
    ...
    for (int i = 0; i < n.m_children; i++) {
        ...
        if (bts >= m_simpquery.m_endTime() ||
            bte <= m_simpquery.m_startTime() ||
            m_except.find(trajid) != m_except.end()) {}
        else {
...
NNEntry *PartsStore::top() {
    ...
    while (true) {
        if (m_mpq.empty() || (!m_nodespq.empty() &&
                              m_nodespq.top()->m_dist < m_mpq.top()->m_dist)) {
...
void PartsStore::Parts::putSBB(xSBB &b) {
    ...
    if (s != it->ts) { //fill prev one
        ...
        if (b.m_startTime <= m_mintime && !m_hasPrev) {
            m_line.insert(it, slab(gaps, gape,
                                   DISTE(m_ps->m_simpquery.frontDistStatic(
                                           b).opt, 1e300, false)));
            m_firstsbb = b;
        } else {
            m_line.insert(it, slab(gaps, gape,
                                   m_ps->m_simpquery.frontDist(b, tjstat->vmax *
                                                                  2)));
        }
    }
    if (e != it->te) { //fill next one
        ...
        if (b.m_endTime >= m_maxtime && !m_hasNext) {
            m_line.insert(nit, slab(gaps, gape,
                                    DISTE(m_ps->m_simpquery.backDistStatic(
                                            b).opt, 1e300, false)));
            m_lastsbb = b;
        } else {
            m_line.insert(nit, slab(gaps, gape,
                                    m_ps->m_simpquery.backDist(b, tjstat->vmax *
                                                                  2)));
        }
    }
    if (s == it->ts) {//try to merge into prev one
        ...
        lit--;
        if (lit != m_line.end() && !lit->d.infer &&
            lit != m_line.begin()) { //prevent the first
            lit->te = e;
        ...
        nnit++;
        if (nit != m_line.end() && !nit->d.infer &&
            nnit != m_line.end()) {//prevent the last
            nit->ts = s;
