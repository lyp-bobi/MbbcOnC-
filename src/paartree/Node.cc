...
uint32_t Node::getByteArraySize(){
    return
            (sizeof(uint32_t) +
             sizeof(uint32_t) +
             sizeof(uint32_t) +
             (m_children * (m_nodeMBRk.getByteArraySize() + sizeof(id_type) + sizeof(uint32_t))) +
             m_totalDataLength +
                    m_nodeMBRk.getByteArraySize());
}

void Node::loadFromByteArray(const uint8_t* ptr){

    m_nodeMBRk=m_pTree->m_infiniteMBRk;
    ...
    {
        m_ptrMBRk[u32Child] = m_pTree->m_MBRkPool.acquire();
        *(m_ptrMBRk[u32Child]) = m_pTree->m_infiniteMBRk;

        m_ptrMBRk[u32Child]->loadFromByteArray(ptr);
        ptr += m_ptrMBRk[u32Child]->getByteArraySize();
    ...
    m_nodeMBRk.loadFromByteArray(ptr);
...
void Node::storeToByteArray(uint8_t** data, uint32_t& len){
    ...
    *data = new uint8_t[len];
    ...
    for (uint32_t u32Child = 0; u32Child < m_children; ++u32Child)
    {
        ...
        m_ptrMBRk[u32Child]->storeToByteArray(&tmpb,tmplen);
    ...
    m_nodeMBRk.storeToByteArray(&tmpb,tmplen);
    memcpy(ptr, tmpb, tmplen);
    //ptr += tmplen;
...
Node::Node(SpatialIndex::PAARTree::PAARTree* pTree, id_type id, uint32_t level, uint32_t capacity) :
        m_pTree(pTree),
        m_level(level),
        m_identifier(id),
        m_children(0),
        ...
        m_totalDataLength(0)
{
    ...
    try
    {
        ...
        m_ptrMBRk = new MBRkPtr[m_capacity + 1];
...
bool Node::insertData(uint32_t dataLength, uint8_t* pData, MBRk& mbbc, id_type id, std::stack<id_type>& pathBuffer, uint8_t* overflowTable)
{
    if (m_children < m_capacity)
    {
        ...
        bool b = m_nodeMBRk.containsMBRk(mbbc);
