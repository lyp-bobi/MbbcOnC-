...
namespace SpatialIndex {
    namespace PAARTree {
        ...
        private:
            Node();
            Node(PAARTree* pTree, id_type id, uint32_t level, uint32_t capacity);

            virtual Node& operator=(const Node&);


            virtual void insertEntry(uint32_t dataLength, uint8_t* pData, MBRk& mbbc, id_type id);
            //virtual void deleteEntry(uint32_t index);

            //virtual bool insertData(uint32_t dataLength, uint8_t* pData, MBRk& mbbc, id_type id, std::stack<id_type>& pathBuffer, uint8_t* overflowTable);
            //virtual void reinsertData(uint32_t dataLength, uint8_t* pData, MBRk& mbbc, id_type id, std::vector<uint32_t>& reinsert, std::vector<uint32_t>& keep);

            //virtual NodePtr chooseSubtree(const MBRk& mbbc, uint32_t level, std::stack<id_type>& pathBuffer) = 0;
            virtual NodePtr findLeaf(const MBRk& mbbc, id_type id, std::stack<id_type>& pathBuffer) = 0;
