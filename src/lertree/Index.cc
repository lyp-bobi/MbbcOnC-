...
Index::Index(SpatialIndex::LERTree::LERTree* pTree, id_type id, uint32_t level) : Node(pTree, id, level, pTree->m_indexCapacity)
{
}
/*
NodePtr Index::chooseSubtree(const Region& mbc, uint32_t insertionLevel, std::stack<id_type>& pathBuffer)
{
...
*/
//NodePtr Index::findLeaf(const Region& mbc, id_type id, std::stack<id_type>& pathBuffer)
//{
//    pathBuffer.push(m_identifier);
//
//    for (uint32_t cChild = 0; cChild < m_children; ++cChild)
//    {
//        if (m_ptrMBR[cChild]->containsRegion(mbc))
//        {
//            NodePtr n = m_pTree->readNode(m_pIdentifier[cChild]);
//            NodePtr l = n->findLeaf(mbc, id, pathBuffer);
...
//}
/*
void Index::split(uint32_t dataLength, uint8_t* pData, Region& mbc, id_type id, NodePtr& ptrLeft, NodePtr& ptrRight)
{
    ...
    switch (m_pTree->m_treeVariant)
    {
        case RV_LINEAR:
        case RV_QUADRATIC:
            rtreeSplit(dataLength, pData, mbc, id, g1, g2);
            break;
        case RV_RSTAR:
            rstarSplit(dataLength, pData, mbc, id, g1, g2);
            break;
        default:
...
uint32_t Index::findLeastEnlargement(const Region& r) const
{
    ...
    ShapePtr t = m_pTree->m_RegionPool.acquire();
...
void Index::adjustTree(Node* n, std::stack<id_type>& pathBuffer)
{
    ...
    // Region needs recalculation if either:
    //   1. the NEW child Region is not contained.
    //   2. the OLD child Region is touching.
    bool bContained = m_nodeMBR.containsRegion(n->m_nodeMBR);
    bool bTouches = m_nodeMBR.touchesRegion(*(m_ptrMBR[child]));
    bool bRecompute = (! bContained || (bTouches && m_pTree->m_bTightRegions));
