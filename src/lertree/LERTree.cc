...
SpatialIndex::LERTree::Data::Data(uint32_t len, uint8_t* pData, MBC& bc, id_type id)
        : m_id(id), m_mbc(bc), m_pData(0), m_dataLength(len)
{
    if (m_dataLength > 0)
    {
...
SpatialIndex::LERTree::Data::~Data()
{
    delete[] m_pData;
}

SpatialIndex::LERTree::Data* SpatialIndex::LERTree::Data::clone()
{
    return new Data(m_dataLength, m_pData, m_mbc, m_id);
}

id_type SpatialIndex::LERTree::Data::getIdentifier() const
{
    return m_id;
}

void SpatialIndex::LERTree::Data::getShape(IShape** out) const
{
    *out = new MBC(m_mbc);
}

void SpatialIndex::LERTree::Data::getData(uint32_t& len, uint8_t** data) const
{
    len = m_dataLength;
    *data = 0;
...
uint32_t SpatialIndex::LERTree::Data::getByteArraySize() const
{
    return
            sizeof(id_type) +
            sizeof(uint32_t) +
...
void SpatialIndex::LERTree::Data::loadFromByteArray(const uint8_t* ptr)
{
...
void SpatialIndex::LERTree::Data::storeToByteArray(uint8_t** data, uint32_t& len)
{
...
SpatialIndex::ISpatialIndex* SpatialIndex::LERTree::returnLERTree(SpatialIndex::IStorageManager& sm, Tools::PropertySet& ps)
{
    SpatialIndex::ISpatialIndex* si = new SpatialIndex::LERTree::LERTree(sm, ps);
    return si;
}

SpatialIndex::ISpatialIndex* SpatialIndex::LERTree::createNewLERTree(
        SpatialIndex::IStorageManager& sm,
        double fillFactor,
        uint32_t indexCapacity,
        uint32_t L1indexCapacity,
        ...
        id_type& indexIdentifier)
{
    ...
    ISpatialIndex* ret = returnLERTree(sm, ps);
...
SpatialIndex::ISpatialIndex* SpatialIndex::LERTree::createAndBulkLoadNewLERTree(
        BulkLoadMethod m,
        IDataStream& stream,
        SpatialIndex::IStorageManager& sm,
        double fillFactor,
        ...
        id_type& indexIdentifier)
{
    SpatialIndex::ISpatialIndex* tree = createNewLERTree(sm, fillFactor, indexCapacity,L1indexCapacity, leafCapacity, dimension, indexIdentifier);

    uint32_t bindex = static_cast<uint32_t>(std::floor(static_cast<double>(indexCapacity * fillFactor)));
    uint32_t bL1index = static_cast<uint32_t>(std::floor(static_cast<double>(L1indexCapacity * fillFactor)));
    uint32_t bleaf = static_cast<uint32_t>(std::floor(static_cast<double>(leafCapacity * fillFactor)));

    SpatialIndex::LERTree::BulkLoader bl;

    stream.rewind();//rewind for reading
    switch (m)
    {
        case BLM_STR:
            bl.bulkLoadUsingSTR(static_cast<LERTree*>(tree), stream, bindex,bL1index, bleaf, 10000, 100);
            break;
        default:
            throw Tools::IllegalArgumentException("createAndBulkLoadNewLERTree: Unknown bulk load method.");
            break;
    }
...
SpatialIndex::ISpatialIndex* SpatialIndex::LERTree::loadLERTree(IStorageManager& sm, id_type indexIdentifier)
{
    Tools::Variant var;
    Tools::PropertySet ps;
    ...
    return returnLERTree(sm, ps);
}

SpatialIndex::LERTree::LERTree::LERTree(IStorageManager& sm, Tools::PropertySet& ps) :
        ...
        m_leafPool(100)
{
    ...
    if (var.m_varType != Tools::VT_EMPTY)
    {
        ...
        else throw Tools::IllegalArgumentException("LERTree: Property IndexIdentifier must be Tools::VT_LONGLONG");
...
SpatialIndex::LERTree::LERTree::~LERTree()
{
#ifdef HAVE_PTHREAD_H
...
void SpatialIndex::LERTree::LERTree::insertData(uint32_t len, const uint8_t* pData, const IShape& shape, id_type id)
{
    throw Tools::NotSupportedException("insertion on LERTree is not supported now");
}

bool SpatialIndex::LERTree::LERTree::deleteData(const SpatialIndex::IShape &shape, SpatialIndex::id_type id) {
    throw Tools::NotSupportedException("deletion on LERTree is not supported now");
}
void SpatialIndex::LERTree::LERTree::containsWhatQuery(const SpatialIndex::IShape &query, SpatialIndex::IVisitor &v) {
    throw Tools::NotSupportedException("not supported now");
}

void SpatialIndex::LERTree::LERTree::intersectsWithQuery(const SpatialIndex::IShape &query, SpatialIndex::IVisitor &v) {
    rangeQuery(IntersectionQuery, query, v);
}

void SpatialIndex::LERTree::LERTree::pointLocationQuery(const SpatialIndex::Point &query, SpatialIndex::IVisitor &v) {
    throw Tools::NotSupportedException("not supported now");
}

void SpatialIndex::LERTree::LERTree::nearestNeighborQuery(uint32_t k, const SpatialIndex::IShape &query,
...
void SpatialIndex::LERTree::LERTree::selfJoinQuery(const SpatialIndex::IShape &s, SpatialIndex::IVisitor &v) {
    throw Tools::NotSupportedException("not supported now");
}

void SpatialIndex::LERTree::LERTree::queryStrategy(IQueryStrategy& qs)
{
#ifdef HAVE_PTHREAD_H
    Tools::LockGuard lock(&m_lock);
#endif
...
void SpatialIndex::LERTree::LERTree::getIndexProperties(Tools::PropertySet& out) const
{
    Tools::Variant var;
...
void SpatialIndex::LERTree::LERTree::addCommand(ICommand* pCommand, CommandType ct)
{
    throw Tools::NotSupportedException("not supported now");
}

bool SpatialIndex::LERTree::LERTree::isIndexValid() {
    return true;
}

void SpatialIndex::LERTree::LERTree::getStatistics(IStatistics** out) const
{
    *out = new Statistics(m_stats);
}


void SpatialIndex::LERTree::LERTree::initNew(Tools::PropertySet& ps)
{
    Tools::Variant var;
...
void SpatialIndex::LERTree::LERTree::initOld(Tools::PropertySet& ps)
{
    loadHeader();
...
void SpatialIndex::LERTree::LERTree::storeHeader()
{
...
void SpatialIndex::LERTree::LERTree::loadHeader()
{
    uint32_t headerSize;
    uint8_t* header = 0;
...
SpatialIndex::LERTree::NodePtr SpatialIndex::LERTree::LERTree::readNode(id_type page)
{
    uint32_t dataLength;
    uint8_t* buffer;
    ...
    try
    {
        ...
        else if (nodeType == PersistentL1Index) n = m_L1indexPool.acquire();
        ...
        if (n.get() == 0)
        {
            ...
            else if (nodeType == PersistentIndex) n = NodePtr(new L1Index(this, -1), &m_indexPool);
...
SpatialIndex::id_type SpatialIndex::LERTree::LERTree::writeNode(Node* n)
{
    uint8_t* buffer;
    uint32_t dataLength;
...
void SpatialIndex::LERTree::LERTree::deleteNode(Node* n)
{
    try
    {
...
/*
void SpatialIndex::LERTree::LERTree::insertData_impl(uint32_t dataLength, uint8_t* pData, Region& mbc, id_type id)
{
    assert(mbc.getDimension() == m_dimension);
    ...
    try
    {
        ...
        NodePtr l = root->chooseSubtree(mbc, 0, pathBuffer);
        if (l.get() == root.get())
        {
            assert(root.unique());
            root.relinquish();
        }
        l->insertData(dataLength, pData, mbc, id, pathBuffer, overflowTable);
...
std::ostream& SpatialIndex::LERTree::operator<<(std::ostream& os, const LERTree& t)
{
...
void SpatialIndex::LERTree::LERTree::rangeQuery(RangeQueryType type, const IShape& query, IVisitor& v)
{
#ifdef HAVE_PTHREAD_H
    Tools::LockGuard lock(&m_lock);
#endif
...
//                }
            ...
            for (uint32_t cChild = 0; cChild < n->m_children; ++cChild)
            {
                MBCs bcs=*n->m_ptrMBCs[cChild].get();
                for(int p=0;p<bcs.m_ids.size();p++) {
                    MBC bc = bcs.m_mbcs[p];
                    bool b;
                    if (type == ContainmentQuery) b = bc.containsShape(query);
                    else b = bc.intersectsShape(query);

                    if (b) {
                        Data data = Data(n->m_pDataLength[cChild], n->m_pData[cChild], *(n->m_ptrMBC[cChild]),
...
//            if(n->m_level<3) {
                for (uint32_t cChild = 0; cChild < n->m_children; ++cChild) {
                    if (n->m_ptrMBR[cChild]->intersectsShape(query)) st.push(readNode(n->m_pIdentifier[cChild]));
                }
//            }else{
//                for (uint32_t cChild = 0; cChild < n->m_children; ++cChild) {
//                    if (n->m_ptrMBR[cChild]->m_wmbr.intersectsShape(query)) st.push(readNode(n->m_pIdentifier[cChild]));
//                }
//            }
        }
    }
